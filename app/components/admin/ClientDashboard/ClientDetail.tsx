...
export default function ClientDetail() {
  ...
  const { data: tasks, isLoading: tasksLoading, refetch: refetchTasks } = useTasks(clientId);
  ...
  if (!client) return (
    <div className="p-[40px] text-center text-[var(--crm-text-muted)]">
      ...
      <h2 className="text-[18px] font-[700] text-[var(--crm-text)]">Không tìm thấy khách hàng</h2>
      <button onClick={() => navigate("/admin/crm")} className="mt-[24px] px-[20px] py-[10px] bg-[var(--crm-navy)] text-white rounded-[var(--crm-r)] font-[600]">
        Quay lại Dashboard
      </button>
    </div>
  );
    ...
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="bg-[var(--crm-navy)] rounded-[var(--crm-r-lg)] p-[24px_28px] mb-[24px] text-white">
        <div className="flex justify-between items-start mb-[18px] gap-[12px]">
          <div>
            <div className="text-[26px] font-[800] line-height-[1.2]">{client.name}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-[16px]">
          <div>
            <div className="text-[11px] text-[rgba(255,255,255,0.55)] font-[600] uppercase tracking-[0.5px] mb-[4px]">Người liên hệ</div>
            <div className="text-[15px] font-[600]">{client.contact_person || "—"}</div>
          </div>
          <div>
            <div className="text-[11px] text-[rgba(255,255,255,0.55)] font-[600] uppercase tracking-[0.5px] mb-[4px]">Điện thoại / Zalo</div>
            <div className="text-[15px] font-[600]">{client.phone || "—"}</div>
          </div>
          <div>
            <div className="text-[11px] text-[rgba(255,255,255,0.55)] font-[600] uppercase tracking-[0.5px] mb-[4px]">Gói dịch vụ</div>
            <div className="text-[15px] font-[600]">{client.service_packages?.name || "—"}</div>
          </div>
          <div>
            <div className="text-[11px] text-[rgba(255,255,255,0.55)] font-[600] uppercase tracking-[0.5px] mb-[4px]">Chi phí / tháng</div>
            <div className="text-[15px] font-[600] text-[var(--crm-gold-light)]">
              {new Intl.NumberFormat("vi-VN").format(Number(client.monthly_cost) || 0)} ₫
            </div>
          </div>
          {client.notes && (
            <div className="col-span-full mt-[8px] pt-[12px] border-t border-[rgba(255,255,255,0.15)]">
              <div className="text-[11px] text-[rgba(255,255,255,0.55)] font-[600] uppercase tracking-[0.5px] mb-[4px]">Ghi chú</div>
              <div className="text-[13px] font-[400] whitespace-pre-wrap">{client.notes}</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-[20px]">
        {(["todo", "doing", "done"] as const).map((status) => (
          <div key={status} className="bg-[var(--crm-surface2)] rounded-[var(--crm-r-lg)] border border-[var(--crm-border)] overflow-hidden">
            <div className={`p-[14px_16px] flex justify-between items-center bg-[var(--crm-surface)] border-b-[3px] ${
              status === 'todo' ? 'border-[#94A3B8]' : 
              status === 'doing' ? 'border-[var(--crm-gold)]' : 
            ...
            >
              {groupedTasks[status].map((task) => {
                ...
                if (task.deadline) {
                  ...
                  if (diff < 0) { dlCls = 'bg-[var(--crm-red-bg)] text-[var(--crm-red)]'; dlIco = '🔴'; }
                  else if (diff <= 3) { dlCls = 'bg-[var(--crm-yellow-bg)] text-[var(--crm-yellow)]'; dlIco = '🟡'; }
                }
                  ...
                  >
                    <div className="flex items-center gap-[6px] mb-[7px]">
                      <div className={`w-[8px] h-[8px] rounded-full ${
                        task.priority === 'high' ? 'bg-[var(--crm-red)]' :
                        task.priority === 'medium' ? 'bg-[var(--crm-yellow)]' :
                        'bg-[var(--crm-green)]'
                      }`} />
                      <span className={`text-[11px] font-[700] uppercase tracking-[0.5px] ${
                        task.priority === 'high' ? 'text-[var(--crm-red)]' :
                        task.priority === 'medium' ? 'text-[var(--crm-yellow)]' :
                        'text-[var(--crm-green)]'
                      }`}>
                    ...
                    <div className="text-[14px] font-[600] text-[var(--crm-text)] mb-[5px] leading-[1.4] line-clamp-2">{task.title}</div>
                    ...
                    <div className="flex items-center justify-between">
                      {task.deadline ? (
                        <span className={`inline-flex items-center gap-[5px] px-[8px] py-[2px] rounded-[20px] text-[11px] font-[600] ${dlCls}`}>
