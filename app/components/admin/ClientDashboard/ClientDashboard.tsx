...
export default function ClientDashboard() {
  ...
  const { data: clientsData, isLoading } = useClients({ search, page, limit: 10 });
  ...
  // Tính stats
  const totalRevenue = clientsData?.data?.reduce((s, c) => s + (Number(c.monthly_cost) || 0), 0) || 0;
    ...
    <div className="max-w-[1440px] mx-auto space-y-[24px]">
      ...
      <div className="flex flex-wrap gap-[12px] items-center">
        <div className="flex-1 min-w-[200px] relative">
          <span className="absolute left-[12px] top-1/2 -translate-y-1/2 text-[var(--crm-text-muted)] pointer-events-none text-[16px]">🔍</span>
          <input
            type="text"
      ...
      {totalCount > 10 && (
        <div className="flex items-center justify-center mt-[24px] gap-[10px]">
          ...
          <span className="text-[14px] font-[600] text-[var(--crm-text)] px-4">Trang {page}</span>
          <button
...
function StatCard({
  ...
  return (
    <div className="bg-[var(--crm-surface)] rounded-[var(--crm-r-lg)] p-[20px_22px] shadow-[var(--crm-shadow)] border border-[var(--crm-border)] flex items-center gap-[16px]">
      <div className={`w-[50px] h-[50px] rounded-[12px] flex items-center justify-center text-[24px] flex-shrink-0 ${iconBgs[colorClass]}`}>
        {icon}
      </div>
      <div>
        <div className="text-[11px] text-[var(--crm-text-muted)] font-[600] uppercase tracking-[0.5px] mb-[3px]">{label}</div>
        ...
        <div className="text-[12px] text-[var(--crm-text-muted)] mt-[3px]">{subLabel}</div>
      </div>
    </div>
  );
}
